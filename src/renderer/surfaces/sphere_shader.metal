...
VertexOut out;
out.
position = uniform.camera * float4(sphere_center + radius * in.position, 1.0);
out.
color = float4(light_sample(sphere_center, in.normal), 1.0);
return
out;
}

fragment float4
fragment_main(VertexOut
in [[stage_in]]) {
return in.
color;
}
