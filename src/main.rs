...
fn main() {
    ...
    event_loop
        .run(move |event, event_loop, cf| match event {
            Event::NewEvents(StartCause::Init) => {
                window = Some(WindowBuilder::new().build(&event_loop).unwrap())
            }
            Event::WindowEvent {
                event: WindowEvent::CloseRequested,
                window_id,
            } if window_id == window.as_ref().unwrap().id() => *cf = ControlFlow::ExitWithCode(0),
            _ => (),
        })
        .unwrap();
}
